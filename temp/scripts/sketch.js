...
function setup() {
  ...
  for (let i = 0; i < 8; i++) {
    ...
    let r = random(5, 90); // let kitten = random(targetimgs);
...
class Cross {
  ...
  clicked(px, py) {
    ...
    if (px > this.x && px < this.x + this.r && py > this.y && py < this.y + this.r) {
      this.kitten = target;
      clickedsound.play(); // random(targetimgs);
    }
  }
  ...
  show() {
    image(this.kitten, this.x, this.y, this.r, this.r); // stroke(255);
    // strokeWeight(4);
...
class Dj {
  ...
  clicked(px, py) {
    ...
    //if (d < this.r) {
    if (px > this.x && px < this.x + this.r && py > this.y && py < this.y + this.r) {
      powersound.play(); // random(targetimgs);
    }
  }
  ...
  show() {
    image(this.dj, this.x, this.y, this.r, 100); // stroke(255);
    // strokeWeight(4);
    // fill(this.brightness, 125);
    // ellipse(this.x, this.y, this.r * 2);
  }
